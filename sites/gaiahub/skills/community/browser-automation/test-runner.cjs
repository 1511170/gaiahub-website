...
const config = {
  ...
  pagesToTest: ['/', '/vision', '/commons', '/hub', '/market', '/bioregions', '/academy', '/certification', '/confederation'],
...
async function testNavigation(page, url) {
  ...
  // Test: Botón CTA
  const ctaButtons = await page.$$eval('a[href^="/"], a[href^="https"], button', buttons =>
    buttons.map(b => ({
  ...
  // Test: Menú móvil (solo en viewport mobile/tablet)
    ...
    for (const pagePath of config.pagesToTest) {
      ...
      for (const viewport of config.viewports) {
        ...
        try {
          ...
          // Test forms (solo en páginas con formularios)
          let formTest = null;
          if ((pagePath === '/confederation' || pagePath === '/contact') && viewport.name === 'desktop') {
